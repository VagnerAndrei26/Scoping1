...
import "../interface/EtherFiLiquid1.sol";
import "../interface/NewEtherFiLiqVault.sol";
...
contract BorrowingTest is IBorrowing,Initializable,UUPSUpgradeable,ReentrancyGuardUpgradeable,OwnableUpgradeable{
    ...
    function depositTokens (
        ...
        ITreasury.DepositResult memory depositResult = treasury.deposit{
            value: param.assetName == AssetName.ETH ? param.depositingAmount : 0}(
                msg.sender, ethPrice, depositTime, param.assetName, param.depositingAmount);
        ...
        // Emit Deposit event
        emit Deposit(
            msg.sender,depositResult.borrowerIndex,param.depositingAmount,normalizedAmount,depositTime,ethPrice,tokensToLend,param.strikePrice,optionFees,param.strikePercent,APR,aBondCr);
    }
    ...
    **/
            ...
            if(depositDetail.withdrawed == false) {                                  
                ...
                if(borrowingHealth > 10000){
                    ...
                    collateralToReturn = (depositedAmountvalue + (options.calculateStrikePriceGains(
                        depositDetail.depositedAmountInETH,depositDetail.strikePrice,ethPrice)));
    ...
    function depositEthToEtherFi(address etherFiAddress) external payable returns(uint256 share){
        share = LiquidityPool(etherFiAddress).deposit{value: msg.value}();
        // console.log(share);
        IERC20(0x35fA164735182de50811E8e2E824cFb9B6118ac2).approve(0xf0bb20865277aBd641a307eCe5Ee04E79073416C, share);
        depositEthToEtherFiLiquidVaults(0x5c135e8eC99557b412b9B4492510dCfBD36066F5,share);
    }

    function approveWeETH(uint256 share) external {
        IERC20(assetAddress[AssetName.WeETH]).approve(0xf0bb20865277aBd641a307eCe5Ee04E79073416C, share);
    }

    function depositEthToKelpDao(address kelpDaoAddress) external payable {
        ILRTDepositPool(kelpDaoAddress).depositETH{value: msg.value}(msg.value,'');
        console.log(IERC20(assetAddress[AssetName.rsETH]).balanceOf(address(this)));
        // IERC20(0x35fA164735182de50811E8e2E824cFb9B6118ac2).transfer(msg.sender, share);
    }

    function depositEthToEtherFiLiquidVaults(address etherFi_LVAddress, uint256 depositShare) public returns(uint256 share){
        // share = EtherFiLiquid1(etherFi_LVAddress).deposit(400000000000000000,msg.sender);
        // share = EtherFiLiquid1(etherFi_LVAddress).multiAssetDeposit(0x35fA164735182de50811E8e2E824cFb9B6118ac2,400000000000000000,msg.sender);
        share = EtherFiLiquidNew(etherFi_LVAddress).deposit{value:0}(
