...
contract GlobalVariables is IGlobalVariables,Initializable,UUPSUpgradeable,ReentrancyGuardUpgradeable,OApp {
    ...
    mapping(IBorrowing.AssetName => CollateralData collateralData) private s_collateralData;// Individual collateral data

    /**
     * @dev Initialize the contract which initializer modifier
     * @param usdaAddress USDa address
     * @param cdsAddress CDS address
     * @param endpoint Endpoint address from Layer Zero
    ...
    /**
     * @dev Modifier to check whether the msg.sender is cds or borrow liquidation address
     */
    ...
    function updateCollateralData(
        ...
        CollateralData memory collateralData) external onlyCoreContracts{
    ...
    function setDstGlobalVariablesAddress(address _globalVariables) public onlyOwner{
        ...
        require(_globalVariables != address(0), 
            "Destination address should not be zero address");
    ...
    function oftOrCollateralReceiveFromOtherChains(
        ...
        bytes memory _payload = abi.encode(
                ...
                oftTransferData.tokensToSend,
                oftTransferData.tokensToSend,
                CDSInterface.LiquidationInfo(0,0,0,0,IBorrowing.AssetName.DUMMY,0),0, 
                oftTransferData,
        ...
        feeForCollateralTransfer = quote(FunctionToDo(1), IBorrowing.AssetName.DUMMY, options, false);
        ...
        if(functionToDo == FunctionToDo.TOKEN_TRANSFER){
            ...
            _options = OptionsBuilder.newOptions().addExecutorLzReceiveOption(450000, 0).addExecutorNativeDropOption(
        ...
        // If we need to get only the Collaterals from dst chain
            ...
            _options = OptionsBuilder.newOptions().addExecutorLzReceiveOption(300000, 0).addExecutorNativeDropOption(
                ...
                uint128(2 * feeForTokenTransfer.nativeFee + (
        ...
        // If we need to get both the Collaterals and USDa from dst chain
            ...
            _options = OptionsBuilder.newOptions().addExecutorLzReceiveOption(700000, 0).addExecutorNativeDropOption(
                ...
                // Since feeForCollateralTransfer includes the fee with native token needed, we are subtracting the native token
                // needed to get the fee alone
                uint128(5 * feeForTokenTransfer.nativeFee + (
                    feeForCollateralTransfer.nativeFee - collateralTokenTransferData.ethToSend)),
                bytes32(uint256(uint160(dstGlobalVariablesAddress)))
            );
        }
        ...
        receipt = _lzSend(dstEid, _payload, _options, _fee, payable(refundAddress));
    }
    /**
    ...
    function sendInternal(        
        ...
        // Calling layer zero send function to send to dst chain
        receipt = _lzSend(_dstEid, _payload, _options, _fee, payable(_refundAddress));
    }
    /**
    ...
    function _lzReceive(
        ...
        if(functionToDo == FunctionToDo.TOKEN_TRANSFER){
            ...
            treasury.transferFundsToGlobal([0,0,0,oftTransferData.tokensToSend]);
        ...
        // If need only collateral or collateral and USDa
        }else if(functionToDo == FunctionToDo.COLLATERAL_TRANSFER || functionToDo == FunctionToDo.BOTH_TRANSFER){
            ...
            _options = OptionsBuilder.newOptions().addExecutorLzReceiveOption(280000, 0).addExecutorNativeDropOption(
                ...
                uint128(collateralTokenTransferData.ethToSend), 
                bytes32(uint256(uint160(collateralTokenTransferData.recipient)))
            );
            ...
            treasury.transferFundsToGlobal([
                collateralTokenTransferData.ethToSend,
                collateralTokenTransferData.weETHToSend,
                collateralTokenTransferData.rsETHToSend,
                oftTransferData.tokensToSend
            ]);
            // Send oft to src chain
            usda.send{ value: _fee.nativeFee}( _sendParam, _fee, address(this));
            // treasury.updateDepositedCollateralAmountInWei(IBorrowing.AssetName.ETH, collateralTokenTransferData.ethToSend);
            ...
            for(uint8 i = 0; i < tokensToSend.length ; i++){
                if(tokensToSend[i] > 0){
                    ...
                    IOFT(borrowInstance.assetAddress(
                        IBorrowing.AssetName(i+2)
                    )).send{ value: _fee.nativeFee}( _sendParam, _fee, address(this));
                    // treasury.updateDepositedCollateralAmountInWei(IBorrowing.AssetName(i+2),tokensToSend[i]);
                }
            }
        }
