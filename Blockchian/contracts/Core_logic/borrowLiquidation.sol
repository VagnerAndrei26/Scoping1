...
contract BorrowLiquidation is IBorrowLiquidation,Initializable,OwnableUpgradeable,UUPSUpgradeable,ReentrancyGuardUpgradeable{
    ...
    function setTreasury(address treasuryAddress) external onlyOwner{
        ...
        require(treasuryAddress != address(0) && isContract(treasuryAddress) != false, "Treasury must be contract address & can't be zero address");
    ...
    function setAdmin(address adminAddress) external onlyOwner{
        // Check whether the input address is not a zero address and EOA
        require(adminAddress != address(0) && isContract(adminAddress) != true, "Admin can't be zero address and contract address");
        admin = adminAddress;
    }
    ...
    function liquidateBorrowPosition(
        ...
        }else if(liquidationType == IBorrowing.LiquidationType.THREE){
    ...
    function liquidationType1(
        ...
        uint64  currentEthPrice,
        uint256 lastCumulativeRate
    ) internal returns(CDSInterface.LiquidationInfo memory){
        ...
        // Check whether the treasury have enough collateral to Liquidate
        require(
            depositDetail.depositedAmount <= omniChainData.totalVolumeOfBorrowersAmountinWei,"Not enough funds in treasury");
        ...
        emit Liquidate(index,liquidationAmountNeeded,cdsProfits,depositDetail.depositedAmountInETH,cds.totalAvailableLiquidationAmount());
    ...
    function liquidationType2(
        ...
        weth.approve(address(wrapper),amount);
