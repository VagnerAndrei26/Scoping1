...
interface ITreasury{
    ...
    function approveTokens(IBorrowing.AssetName assetName,address spender, uint amount) external;
    ...
    function updateTotalVolumeOfBorrowersAmountinUSD(uint256 amountInUSD) external;
    function updateDepositedCollateralAmountInWei(IBorrowing.AssetName asset,uint256 amount) external;
    function updateDepositedCollateralAmountInUsd(IBorrowing.AssetName asset, uint256 amountInUSD) external;
    function transferFundsToGlobal(uint256[4] memory transferAmounts) external;
    function depositedCollateralAmountInWei(IBorrowing.AssetName) external view returns(uint256);
    function liquidatedCollateralAmountInWei(IBorrowing.AssetName) external view returns(uint256);
    function withdrawFromExternalProtocolDuringLiq(address user, uint64 index) external returns(uint256);
